...
def stream_chat(req: ChatRequest):
    ...
    def generate_chunks():
        if not api_key:
            # Mock generator fallback when API key is missing
            mock_text = (
                f"### DevPilot Codebase Assistant\n\n"
                f"I have scanned the files in project **{req.project_id}** and compiled the context.\n\n"
                f"Because no `GEMINI_API_KEY` is configured in the python environment, I am answering in **Offline Mock Mode**.\n\n"
        ...
        try:
            ...
            yield f"\n\n[DevPilot Error]: Failed to contact Gemini API. Details: {str(e)}"
...
def generate_docs(req: DocRequest):
    ...
    def generate_chunks():
        ...
        try:
            ...
            response = model.generate_content("Generate documentation aspect", stream=True)
            for chunk in response:
                if chunk.text:
                    yield chunk.text
        except Exception as e:
            yield f"\n\n[Documentation Error]: Failed to generate from Gemini API. Details: {str(e)}"
...
def generate_tests(req: TestRequest):
    ...
    def generate_chunks():
        ...
        try:
            ...
            response = model.generate_content("Generate code test suite", stream=True)
            for chunk in response:
                if chunk.text:
                    yield chunk.text
        except Exception as e:
            yield f"\n\n[Test Generator Error]: Failed to generate test from Gemini. Details: {str(e)}"
...
def generate_diagrams(req: DiagramRequest):
    ...
    if not api_key:
        # Mock diagrams fallback configurations
        ...
        mock_diagrams = {
            ...
            "component": (
                ...
                "  end\n"
                "  subgraph AIService [fastapi-server Python Backend]\n"
                "    API[FastAPI Router]\n"
                "    Scanner[Static Code Analyzer]\n"
                "  end\n"
    ...
    try:
        ...
        response = model.generate_content([system_prompt, "Generate Mermaid.js config"])
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate diagram from Gemini: {str(e)}")
...
def get_analytics(project_id: str):
    ...
    try:
        ...
        security_score = max(15, 100 - (critical * 25 + high * 15 + medium * 8 + low * 4))
        maintainability_score = max(10, 100 - (unused_vars * 2 + large_funcs * 6 + naming_issues * 3 + duplicate_blocks * 5 + nested_loops * 5))
        ...
        doc_score = min(100, doc_score_base + (20 if len(code_findings) < 15 else 10))
        
        complexity_score = max(15, 100 - (nested_loops * 8 + large_funcs * 5))
